...
export const FlexibleFixedLayout: FlowComponent<FixedAndFlexiblePaneLayoutProps> = props => {
  const maximumWidth = createMemo(() => props.maximumWidth - SPACER_WIDTH);

  const snapWidths = () => [0, ...props.snapWidths, maximumWidth()].toReversed();

  const offsetLeft = () => clamp(0, props.maximumWidth - props.width - SPACER_WIDTH, SPACER_WIDTH);
  const offsetRight = () => clamp(0, props.width, SPACER_WIDTH);

  const clampedWidth = () => clamp(0, props.width, maximumWidth());
  const rightPaneWidth = () => clampedWidth() - (offsetLeft() - SPACER_WIDTH);
